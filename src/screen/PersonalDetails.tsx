...
const PersonalDetails = () => {
  ...
  const validateForm = () => {
    ...
    if (!phone.trim() || !/^\d{10,15}$/.test(phone)) errors.push("Valid phone number is required (10-15 digits)");
    if (!address.trim()) errors.push("Address is required");
    if (!zipCode.trim()) errors.push("Zip code is required");
    if (!cityId) errors.push("City is required");
    if (!selectedFile) errors.push("Profile picture is required");
        ...
        {fetching ? (
          ...
          <>
            ...
            <View style={styles.profileSection}>
              <TouchableOpacity onPress={openImagePicker} disabled={loading}>
                <View style={styles.profileImageContainer}>
                  {selectedFile ? (
                    <Image source={{ uri: selectedFile.uri }} style={styles.profileImage} />
                  ) : (
            ...
            >
              {loading ? (
                ...
                <Text style={styles.submitButtonText}>
                  {customerData ? 'Update Profile' : 'Save Profile'}
                </Text>
              )}
            </TouchableOpacity>
          </>
